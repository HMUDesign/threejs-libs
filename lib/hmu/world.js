...
//				console.log(width, height, width / height)
				...
				this.box = new THREE.Mesh(new THREE.PlaneGeometry(width, height), new THREE.MeshBasicMaterial({ wireframe: true }));
	...
	World.prototype.setHeight = function(height) {
		this.height = height;
...
//		document.body.style.height = (100 * this.height / height) + '%';
	...
	World.prototype.scroll = function() {
		var event = {
			type: 'scroll',
		...
		var shiftfov = - Math.tan(THREE.Math.degToRad(this.camera.fov / 2)) * this.camera.position.z;
		var shiftscroll = - event.percent * (this.height ? (this.height - height) : 1);
		
		console.log(shiftfov)
		
		this.camera.position.y = shiftfov + shiftscroll;
